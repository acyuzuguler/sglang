...
# (SGLANG_CREDIT_{DECODE,PREFILL}_{MAX_CRED,COST}, SGLANG_CREDIT_DECODE_BETA,
# SGLANG_CREDIT_PREFILL_PROTECT):
#
# - DECODE (CUDA-graph safe; mirrors the sim's select_expert_credit): every request holds
#   an int credit balance per (layer, expert) in `creds`, initialized to decode_max_cred
#   (and reset to it by every extend chunk, so decode ALWAYS starts fresh after prefill)
#   and regenerated by +1 per decoded token. Selecting an expert costs decode_cost.
...
#   while high-margin picks survive.
#
...
class CreditRouter:
    ...
    def __init__(
        ...
        *,
        ...
        # Decode-phase credits (see header). Prefill keeps no cross-forward state.
        ...
        # Device-side max_cred row for the in-graph decode-buffer reset: assigning a
        # Python scalar to a CUDA slice is an illegal CPU->CUDA copy during CUDA-graph
        # capture, so we keep a preallocated on-device value to broadcast instead.
        self.max_cred_row = torch.full(
            (num_experts,), decode_max_cred, dtype=torch.int32, device=device
        )
    ...
    def _route_decode(
        ...
        creds = torch.clamp(self.creds[safe_idx, layer_id, :] + valid_i, max=self.decode_max_cred)
        ...
        spend = torch.zeros_like(creds).scatter_(
            1, ids, (self.decode_cost * valid_i).expand(-1, self.top_k)
        )
